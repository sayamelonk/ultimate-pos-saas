    ...
    <div class="flex items-center justify-between h-16 px-6">
        ...
        <div class="flex items-center gap-3">
            @if(auth()->check())
                ...
                @else
                    <!-- Tenant & Outlet Info for Regular Users -->
                    @php
                    ...
                    {{-- Tenant Name Badge --}}
                    @if($userTenant)
                    <div class="hidden md:flex items-center gap-2 px-3 py-1.5 bg-primary-50 border border-primary-200 rounded-lg">
                        <svg class="w-4 h-4 text-primary" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                  d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4"/>
                        </svg>
                        <span class="text-sm font-semibold text-primary">{{ $userTenant->name }}</span>
                    </div>
                    @endif

                    {{-- Outlet Selector --}}
                    @php
                        // For tenant-owner, show all tenant outlets
                        $isTenantOwner = auth()->user()->roles()->where('slug', 'tenant-owner')->exists();
                        $availableOutlets = $isTenantOwner && $userTenant
                            ? $userTenant->outlets()->where('is_active', true)->get()
                            : $userOutlets;
                    @endphp
                    @if($availableOutlets->count() > 0)
                    <div x-data="{ open: false }" class="relative">
                        <button @click="open = !open"
                                class="flex items-center gap-2 px-3 py-1.5 rounded-lg border border-border hover:bg-secondary-50 transition-colors">
                            ...
                            <span class="hidden sm:block text-sm font-medium text-secondary-700">
                                {{ $currentOutlet?->name ?? __('app.no_outlet') }}
                            </span>
                        ...
                        <div x-show="open"
                            ...
                            <div class="px-4 py-2 text-xs font-semibold text-muted uppercase tracking-wider border-b border-border">
                                {{ __('menu.select_outlet') }}
                            </div>
                            @foreach($availableOutlets as $outlet)
                                @php
                                    $isCurrentOutlet = $currentOutlet && $currentOutlet->id === $outlet->id;
                                @endphp
                                <form action="{{ route('admin.outlets.switch', $outlet) }}" method="POST">
                                    @csrf
                                    <button type="submit" class="flex items-center gap-3 w-full px-4 py-2 hover:bg-secondary-50 text-sm text-left {{ $isCurrentOutlet ? 'bg-primary-50' : 'text-secondary-600' }}">
                                        <span class="w-2 h-2 rounded-full {{ $isCurrentOutlet ? 'bg-primary' : 'bg-secondary-300' }}"></span>
                                        <span class="flex-1">{{ $outlet->name }}</span>
                                        @if($isCurrentOutlet)
                                            <svg class="w-4 h-4 text-primary" fill="currentColor" viewBox="0 0 20 20">
                                                <path fill-rule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clip-rule="evenodd"/>
                                            </svg>
            ...
            <div x-data="{ open: false }" class="relative">
                <button @click="open = !open"
                    ...
                    <div class="hidden sm:block text-left">
                        ...
                        <p class="text-xs text-muted">{{ auth()->check() ? (auth()->user()->roles->first()?->name ?? 'User') : '' }}</p>
                    </div>
                    <svg class="w-4 h-4 text-secondary-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"/>
                    </svg>
                ...
                <div x-show="open"
                    ...
                    <a href="{{ route('admin.users.edit', auth()->id()) }}" class="flex items-center gap-3 px-4 py-2 hover:bg-secondary-50 text-sm text-secondary-700">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    ...
                    @if(!auth()->user()->isSuperAdmin())
                    <a href="{{ route('subscription.index') }}" class="flex items-center gap-3 px-4 py-2 hover:bg-secondary-50 text-sm text-secondary-700">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                  d="M3 10h18M7 15h1m4 0h1m-7 4h12a3 3 0 003-3V8a3 3 0 00-3-3H6a3 3 0 00-3 3v8a3 3 0 003 3z"/>
                        </svg>
                        {{ __('menu.subscription') }}
                    </a>
                    @endif
