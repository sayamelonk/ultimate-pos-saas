<x-app-layout>
    <x-slot name="title">{{ __('products.add_modifier_title') }} - Ultimate POS</x-slot>

    @section('page-title', __('products.add_modifier_title'))
    ...
    <x-slot name="header">
        <div class="flex items-center gap-4">
            ...
            <div>
                <h2 class="text-2xl font-bold text-text">{{ __('products.add_modifier_title') }}</h2>
                <p class="text-muted mt-1">{{ __('products.add_modifier_options') }}</p>
            </div>
        </div>
    </x-slot>
        ...
        <div class="grid grid-cols-3 gap-6">
            <div class="col-span-2 space-y-6">
                ...
                <x-card title="{{ __('products.modifiers') }}">
                    <p class="text-muted mb-4">{{ __('products.add_modifier_options') }}</p>
            ...
            <div class="space-y-6">
                ...
                <x-card title="{{ __('products.preview') }}">
                    <div class="space-y-3">
                        <p class="text-sm text-muted">{{ __('products.how_appears_pos') }}</p>

                        <div class="p-3 bg-secondary-50 rounded-lg">
                            <p class="font-medium mb-2" x-text="'{{ old('name') }}' || '{{ __('products.group_name') }}'"></p>
                            ...
                            <div x-show="selectionType === 'single'" class="space-y-2">
                                <template x-for="(modifier, index) in modifiers" :key="index">
                                    <label class="flex items-center gap-2 cursor-pointer">
                                        ...
                                        <span x-text="modifier.name || '{{ __('products.modifier') }} ' + (index + 1)"></span>
                ...
                <!-- Examples -->
                <x-card title="{{ __('products.common_examples') }}">
                    <div class="space-y-3 text-sm text-muted">
                        <div>
                            <p class="font-medium text-text">{{ __('products.toppings_multi') }}</p>
                            <p>Cheese, Bacon, Onion, Mushroom</p>
                        </div>
                        <div>
                            <p class="font-medium text-text">{{ __('products.extra_shots_multi') }}</p>
                            <p>Espresso +5k, Vanilla +3k, Caramel +3k</p>
                        </div>
                        <div>
                            <p class="font-medium text-text">{{ __('products.sauce_single') }}</p>
                            <p>BBQ, Mayo, Ketchup, Mustard</p>
                        </div>
                        <div>
                            <p class="font-medium text-text">{{ __('products.milk_type_single') }}</p>
    ...
    <script>
        function modifierGroupForm() {
            const defaultModifier = [{name: '', price: 0, inventory_item_id: ''}];
            const oldModifiers = @json(old('modifiers'));
            ...
            return {
                ...
                modifiers: oldModifiers || defaultModifier,
